...
function ClientCard({
  ...
  return (
    <div
      ...
      className="group rounded-xl border border-border/60 bg-background p-3 shadow-sm transition-all hover:border-primary/25 hover:shadow-md cursor-grab active:cursor-grabbing active:scale-[0.98] select-none"
    >
...
function BoardCustomizeDialog({
  ...
  const renameColumn = (id: string, label: string) => {
    ...
    setDraft({
      ...draft,
      columns: { ...draft.columns, [id]: { ...col, label } },
    });
  };
  const hideColumn = (id: string) => {
    ...
    setDraft({
      ...draft,
      columns: { ...draft.columns, [id]: { ...col, hidden: true } },
    });
  };
  const showHiddenColumn = (id: string) => {
    ...
    setDraft({
      ...draft,
      columns: { ...draft.columns, [id]: { ...col, hidden: false } },
    });
  };
  ...
  const handleAddColumn = () => {
    ...
    const next = addCustomColumn(draft, newLabel, newStatusKey);
    setDraft(next);
    setNewLabel("");
    setNewStatusKey("");
  };
        ...
        {available.length > 0 && (
          <div className="space-y-2 rounded-lg border border-border/70 bg-muted/20 p-3">
            ...
            <p className="text-[11px] text-muted-foreground">
              Pick a workflow stage not already on the board. You can give it a custom name.
            </p>
            <Input
              value={newLabel}
    ...
    <div className="space-y-5 pb-10">
      ...
      <section className="rounded-2xl border border-border/70 bg-gradient-to-br from-background via-background to-primary/5 p-5 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
          {[
            { label: "Active clients", value: stats.total, icon: Users },
            ...
            <div
              key={s.label}
              className="flex items-center gap-2.5 rounded-xl border border-border/60 bg-background/80 px-3 py-2.5"
            >
      ...
      {pipeline.length > 0 && (
        <div className="relative max-w-md">
          ...
          <Input
            ...
            placeholder="Search client name, email, or pathway…"
            className="h-9 rounded-xl border-border/70 bg-background pl-9 text-sm"
          />
        </div>
      )}
      {pipeline.length === 0 ? (
            ...
            {visibleColumns.map((col) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "flex min-h-[min(28rem,65vh)] min-w-0 flex-col overflow-hidden rounded-xl border border-border/80 bg-card shadow-sm transition-all",
                ...
                >
                  <div className={cn("flex items-center gap-2.5 border-b px-3 py-3", theme.accentBar)}>
                    ...
                    <div className="min-w-0 flex-1">
                      <p className={cn("truncate text-sm font-semibold leading-tight", theme.headerClass)}>
                        {col.label}
                      </p>
                  ...
                  <div className="flex flex-1 flex-col gap-2 overflow-y-auto bg-muted/10 p-2.5">
                    ...
                    {cards.length === 0 && (
                      <div
                        className={cn(
                          "flex flex-1 items-center justify-center rounded-lg border border-dashed px-3 py-10",
                          theme.dropZoneClass,
                        )}
                      >
                        <p className="text-center text-xs text-muted-foreground">
                          {search ? "No matches" : "Drop client here"}
                        </p>
                      </div>
                    )}
                  </div>
