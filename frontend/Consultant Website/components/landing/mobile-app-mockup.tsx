...
export function MobileAppMockup() {
  return (
    <div className="relative mx-auto w-[260px] sm:w-[280px]">
      <div className="absolute -inset-6 rounded-[3rem] bg-gradient-to-br from-red-500/20 via-red-500/10 to-transparent blur-2xl" />
      <div className="relative rounded-[2.5rem] border-[6px] border-slate-800 bg-slate-900 p-2 shadow-2xl">
        ...
        <div className="overflow-hidden rounded-[2rem] bg-gradient-to-b from-red-50 to-white">
          ...
          <div className="space-y-2 p-3">
            {[
              { icon: Users, label: "3 new client invites", color: "text-[#c8102e]" },
              ...
              {["Clients", "Cases", "Meetings", "Payments"].map((tab, i) => (
                <div
                  ...
                  className={`rounded-lg py-2 text-center text-[10px] font-semibold ${
                    i === 0 ? "bg-[#c8102e] text-white" : "bg-muted text-muted-foreground"
                  }`}
                >
                  {tab}
                </div>
      ...
      <div className="absolute -right-6 bottom-12 hidden rounded-2xl border border-red-200 bg-white px-3 py-2 shadow-lg sm:flex sm:items-center sm:gap-2">
        <Smartphone className="h-4 w-4 text-[#c8102e]" />
        <span className="text-xs font-semibold text-[#a00d24]">iOS & Android</span>
      </div>
    </div>
  );
}
