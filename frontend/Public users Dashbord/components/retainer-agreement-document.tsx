...
function SectionTitle({ n, children }: { n: number; children: React.ReactNode }) {
  return (
    <div className="mb-2 flex items-center gap-2">
      <p className="text-xs font-bold uppercase tracking-wide text-primary">{n}. {children}</p>
    </div>
  );
}

function SectionOverride({ text, children }: { text?: string; children: React.ReactNode }) {
          ...
          <>
            ...
            {config.taxEnabled && config.taxRate > 0 && (
              <div className="mt-3 border-t border-foreground/10 pt-3 text-xs">
                <div className="flex justify-between gap-3"><span>Professional fee</span><span>{formatAgreementCurrency(config.totalFee, config.currency)}</span></div>
                <div className="mt-1 flex justify-between gap-3"><span>{config.taxLabel} ({config.taxRate}%)</span><span>{formatAgreementCurrency(taxAmount, config.currency)}</span></div>
                <div className="mt-1 flex justify-between gap-3 border-t border-foreground/10 pt-1 font-bold"><span>Total amount payable</span><span>{formatAgreementCurrency(grandTotal, config.currency)}</span></div>
              </div>
            )}
          </>
        </SectionOverride>
